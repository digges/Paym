document.addEventListener("DOMContentLoaded", function () {
  ...
  payButton.addEventListener("click", function () {
    ...
    fetch("http://localhost:8084/api/payment/create-Order", {
      method: "POST",
      headers: {
      ...
      .then((data) => {
        ...
        const options = {
          ...
          handler: function (response) {
            ...
            fetch("http://localhost:8084/api/payment/update-Order", {
              method: "POST",
              headers: {
              ...
              .then((updateData) => {
                ...
                alert(
                  `🎉 Thank you for enrolling in Spark 3.0!\nYour payment of ₹${currentPrice} was successful. A confirmation email has been sent to ${email}.`
                );
              })
              .catch((updateErr) => {
                console.error("Update error:", updateErr);
                alert("Payment succeeded but failed to send confirmation email. Please contact support with payment ID: " + response.razorpay_payment_id);
                payButton.innerHTML = `<i class="fas fa-lock"></i> Pay ₹${currentPrice}`;
                payButton.disabled = false;
              });
          },
